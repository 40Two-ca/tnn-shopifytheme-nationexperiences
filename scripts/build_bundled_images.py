...
DISPATCHER = """{%- doc -%}
  ...
  assets/photo-*, assets/tnn-*) from a key, so the network's logos, photography
  and portraits render before anyone has uploaded them to Shopify Files. A real
  image picked in the editor always wins: sections only fall back to this.

  One case per family, in bundled-image-<family>.liquid. It was a single case
  over every key until that tripped theme check's LiquidComplexity rule at 133
  against a maximum of 120.
...
{%- liquid
  assign family = key | split: '-' | first
  case family
    when 'identity'
      render 'bundled-image-identity', key: key, class: class, alt: alt, style: style, loading: loading
    when 'brand'
      render 'bundled-image-brand', key: key, class: class, alt: alt, style: style, loading: loading
    when 'partner'
      render 'bundled-image-partner', key: key, class: class, alt: alt, style: style, loading: loading
    when 'team'
      render 'bundled-image-team', key: key, class: class, alt: alt, style: style, loading: loading
    when 'photo'
      render 'bundled-image-photo', key: key, class: class, alt: alt, style: style, loading: loading
  endcase
-%}
"""

FAMILY_HEAD = """{%- doc -%}
  The __FAMILY__ family for snippets/bundled-image.liquid. Generated by
...
def write_snippets(snippets_dir: str, entries: list) -> None:
    """One dispatcher plus a snippet per family, all generated."""
    families = {}
    for entry in entries:
        families.setdefault(entry[0].split("-")[0], []).append(entry)

    io.open(os.path.join(snippets_dir, "bundled-image.liquid"), "w",
            encoding="utf-8", newline="\n").write(DISPATCHER)
    for family, rows in sorted(families.items()):
        path = os.path.join(snippets_dir, "bundled-image-%s.liquid" % family)
        # .replace rather than %-formatting: Liquid's `{%- doc -%}` reads as a
        # `%d` conversion spec to Python and raises.
        write_snippet(path, rows, head=FAMILY_HEAD.replace("__FAMILY__", family))
        print("  %-34s %d keys" % (os.path.basename(path), len(rows)))
    print("snippet: bundled-image.liquid dispatches %d families" % len(families))


def write_snippet(path: str, entries: list, head: str = None) -> None:
...
def main() -> int:
    ...
    write_snippets(os.path.join(REPO, "snippets"), entries)
